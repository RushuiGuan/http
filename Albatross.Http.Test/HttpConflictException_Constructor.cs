...
namespace Albatross.Http.Test {
	public class HttpConflictException_Constructor {
		[Fact]
		public void CapturesResponseContextAndStatus() {
			var ex = new HttpConflictException(HttpMethod.Post,
				new Uri("https://example.com/api/widgets/5"),
				new HttpResponseContent { ContentType = "application/json", Content = "{\"message\":\"conflict\"}" });

			Assert.Equal(409, ((IHttpException)ex).Status);
			...
			Assert.Equal("{\"message\":\"conflict\"}", ex.Content);
		}

		[Fact]
		public void IsCatchableAsSemanticBaseType() {
			var ex = new HttpConflictException(HttpMethod.Get,
				new Uri("https://example.com/api/test"), new HttpResponseContent());

			Assert.IsAssignableFrom<Albatross.Exceptions.ConflictException>(ex);
		}
	}
}
