...
namespace Albatross.Http.Test {
	public class TestCreateUrlArray {
		[Fact]
		public void TestRequestGeneration1() {
			...
			var queryString = new NameValueCollection {
				...
				{ "isEquity", Convert.ToString(false) }
			};
			var arrayQueryString = new List<string>();
			foreach (var item in ids) {
				arrayQueryString.Add(Convert.ToString(@item));
			}
			var requests = UrlExtensions.CreateUrlArray(baseUri, path, queryString, 2000, "id", arrayQueryString.ToArray());
		...
		public void TestRequestGeneration3(int maxlength, int count, params string[] expected) {
			...
			var requests = UrlExtensions.CreateUrlArray(baseUri, path, queryString, maxlength, "id", arrayQueryString.ToArray()).ToArray();
		...
		public void TestInsufficientMaxLength(int maxlength, int count) {
			...
			Assert.Throws<ArgumentException>(() => UrlExtensions.CreateUrlArray(baseUri, path, queryString, maxlength, "id", arrayQueryString.ToArray()));
		}

		[Fact]
		public void CreateUrlArray_ValueTooLongInMiddleOfArray_Throws() {
			var baseUri = new Uri("http://myyyhost");
			string path = "api/bar";
			var queryString = new NameValueCollection {
				{ "date", "2022-10-10" }
			...
			Assert.Throws<ArgumentException>(() => UrlExtensions.CreateUrlArray(baseUri, path, queryString, maxlength, "id", arrayValues).ToArray());
		}
